/**
 * File: main.cpp
 * Author: Shancang Li & Benedict R. Gaster
 * Date: 21-07-2018
...
int main()
{
    ...
    char out1 [8], out2[8], out3[8], out4[8];
    char outputStr [80];
    ...
    p=(uint8_t *)&h0;
    sprintf(out1, "%2.2x%2.2x%2.2x%2.2x", p[0], p[1], p[2], p[3], h0);
    //uBit.display.scrollAsync(out1);
    //outputStr = out1;

    p=(uint8_t *)&h1;
    sprintf(out2, "%2.2x%2.2x%2.2x%2.2x", p[0], p[1], p[2], p[3], h1);
    //uBit.display.scrollAsync(outputStr);

    p=(uint8_t *)&h2;
    sprintf(out3, "%2.2x%2.2x%2.2x%2.2x", p[0], p[1], p[2], p[3], h2);
    //uBit.display.scrollAsync(outputStr);

    p=(uint8_t *)&h3;
    sprintf(out4, "%2.2x%2.2x%2.2x%2.2x", p[0], p[1], p[2], p[3], h3);
    //uBit.display.scrollAsync(outputStr);
    ...
    for (int i = 0; i<8; i++){
      outputStr[i+8] = out2[i];
    }
    for (int i = 0; i<8; i++){
      outputStr[i+16] = out3[i];
    }
    for (int i = 0; i<8; i++){
      outputStr[i+24] = out4[i];
    }
